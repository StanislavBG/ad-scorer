...
// Local copies of `~/Projects/Bilko/src/components/tool-page/*` with AdScorer's emerald
// theme baked in. First standalone to inline CompareLayout + Rewrites — keep those
// faithful to host source so visual parity is exact.
...
// ─────────────────────────────────────────────────────────────
// ToolHero — emerald theme baked in (was: getToolTheme('ad-scorer')).
// AdScorer's page renders its own 3-tab toggle (Score / Compare / Generate)
...
export function ToolHero({ title, tagline, children }: {
  ...
  return (
    <section className="relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-[#0d1f17] via-[#0a1510] to-[#0d1f17]" />
      <div
        className="absolute inset-0"
        style={{ background: 'radial-gradient(ellipse at center, rgba(16,185,129,0.14), transparent 70%)' }}
      />
...
// ─────────────────────────────────────────────────────────────
// ScoreCard — emerald theme baked in.
...
export function ScoreCard({ score, grade, verdict, toolName }: {
  ...
  return (
    <div className="bg-gradient-to-br from-[#0d1f17] via-[#0a1510] to-[#0d1f17] rounded-2xl p-6 md:p-8 text-center relative overflow-hidden animate-slide-up shadow-dark-elevation">
      <div
        className="absolute inset-0"
        style={{ background: 'radial-gradient(circle at 50% 30%, rgba(16,185,129,0.14), transparent 60%)' }}
      />
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/[0.06] to-transparent" />
      <div className="relative">
        <p className="text-label text-emerald-400 mb-4">{toolName}</p>
        ...
        <p className="text-emerald-400 font-bold italic text-sm md:text-base max-w-md mx-auto mb-5">
          &ldquo;{verdict}&rdquo;
        </p>
